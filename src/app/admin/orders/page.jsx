...
import { 
  Search, FileText, Download, MoreHorizontal, ChevronLeft, ChevronRight, 
...
export default function OrderHistory() {
  ...
  const [period, setPeriod] = useState('all'); // Default ke 'all' agar semua data langsung terlihat
  ...
  // Inisialisasi Auth & Listener Sinkronisasi Dinamis
  useEffect(() => {
    setIsMounted(true);
    ...
    // Listener saat kasir menambah transaksi baru
  ...
  // Filter Periode Berdasarkan Timestamp Transaksi
      ...
      <div className={`max-w-7xl mx-auto px-4 sm:px-6 pt-6 sm:pt-8 relative z-10 space-y-6 transition-all duration-300 ${isSidebarOpen ? 'opacity-40 blur-sm pointer-events-none' : ''}`}>
        ...
        <div className="bg-white/80 backdrop-blur-md rounded-3xl border-2 border-slate-100 p-4 sm:p-5 sm:px-7 shadow-sm flex items-center justify-between gap-4 print:hidden">
          <div className="flex items-center gap-3 sm:gap-4">
            ...
            <div>
              ...
              <p className="text-[10px] sm:text-xs font-bold text-slate-400 mt-1 hidden sm:block">Laporan transaksi & audit penjualan harian.</p>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white rounded-3xl border-2 border-slate-100 shadow-[0_8px_30px_rgb(0,0,0,0.04)] overflow-visible print:border-slate-300 print:shadow-none print:rounded-none">
                ...
                {filteredData.length === 0 ? (
                  <tr>
                    <td colSpan="6" className="text-center py-12 text-slate-400 font-bold text-sm">
                      Belum ada transaksi di periode ini. Lakukan pembayaran di Kasir POS untuk menguji.
                    </td>
                  </tr>
                ) : (
                  filteredData.map((row) => (
                    <tr key={row.id} className="hover:bg-slate-50/50 transition-colors group">
                      ...
                      <td className="p-4">
                        <span className="text-xs font-bold text-slate-800 block uppercase">{row.name}</span>
                      </td>
                      <td className="p-4">
                        <span className="text-xs font-black text-slate-900 block">{row.type}</span>
                        <span className="text-[9px] font-bold text-slate-400 block mt-0.5">
                          {row.toppings ? `Top: ${row.toppings}` : ''}
                        </span>
                      </td>
                      <td className="p-4">
                        ...
                        {activeDropdown === row.id && (
                          <>
                            ...
                            <div className="absolute right-8 top-10 z-20 w-36 bg-white border border-slate-100 rounded-xl shadow-[0_8px_30px_rgb(0,0,0,0.12)] overflow-hidden flex flex-col py-1 animate-in fade-in zoom-in-95 duration-100">
                              ...
                              <button onClick={() => handlePrintStruk(row)} className="flex items-center gap-2 px-3 py-2 text-[10px] font-bold text-slate-600 hover:bg-slate-50 hover:text-amber-600 transition-colors text-left cursor-pointer">
          ...
          <div className="p-4 sm:p-5 border-t border-slate-100 flex items-center justify-between bg-slate-50/30 rounded-b-3xl print:hidden">
            ...
            <button 
              onClick={() => {
                if (confirm('Bersihkan seluruh riwayat transaksi?')) {
            ...
            <div className="p-6 space-y-4 max-h-[70vh] overflow-y-auto [&::-webkit-scrollbar]:hidden">
              <div className="flex justify-between items-end pb-3 border-b border-slate-100">
                <div>
                  ...
                  <p className="text-sm font-black text-slate-800 uppercase">{selectedOrder.name}</p>
                </div>
              ...
              {selectedOrder.items && Array.isArray(selectedOrder.items) ? (
                <div className="space-y-2.5">
                  ...
                  {selectedOrder.items.map((it, idx) => (
                    <div key={idx} className="bg-slate-50 p-3 rounded-xl border border-slate-100 space-y-1">
                      ...
                      <p className="text-[9px] text-slate-400">{it.veg} • {it.spicy}</p>
                ...
                <div className="space-y-2 bg-slate-50 p-4 rounded-2xl border border-slate-100">
                  <p className="text-sm font-black text-slate-800 mb-1">{selectedOrder.type}</p>
